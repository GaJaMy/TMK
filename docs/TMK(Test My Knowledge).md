...
- OpenAI Codex 기반 AI 에이전트를 활용해 구현 보조, 문서 정리, 테스트 보강 작업을 함께 진행했습니다.
- `tmk-user-web`, `tmk-admin-web`는 서비스 흐름 검증과 시연을 위한 데모용 프론트엔드로 함께 개발했습니다.

### 핵심 기술 특징

- **RAG 기반 문제 생성**: 문서를 청킹 → OpenAI 임베딩(1536차원) → pgvector ANN 검색 → LLM 문제 생성
- **자동 채점**: Spring Batch가 초 단위로 만료된 시험을 감지하여 자동 제출 및 채점
- **간소화된 인증**: 아이디 / 비밀번호 기반 회원가입 및 로그인
- **캐시 최적화**: Redis로 refresh token 저장과 access token blacklist 관리
...
| AI | OpenAI API | embedding/chat model은 환경변수 주입 |
...
- `tmk-user-web`: 사용자 인증, 문서 등록, 시험 생성/시작/응시/결과
...
| 구분 | 기능 | 구현 여부 | 비고 |
|------|------|-----------|------|
| 서버 | 회원가입 | 구현됨 | `POST /api/auth/v1/register` |
| 서버 | 로그인 | 구현됨 | `POST /api/auth/v1/login` |
| 서버 | 로그아웃 | 구현됨 | `POST /api/auth/v1/logout` |
| 서버 | JWT 재발급 | 구현됨 | 사용자 `/api/auth/v1/reissue`, 관리자 `/admin/auth/v1/reissue` |
| 서버 | 비밀번호 재설정 | 구현됨 | `POST /api/auth/v1/reset-password` |
| 서버 | 관리자 로그인 | 구현됨 | `POST /admin/auth/v1/login` |
| 서버 | 관리자 목록 조회 | 구현됨 | `GET /admin/v1/users` |
...
| 서버 | 공용 문제 상세 조회 | 구현됨 | `GET /admin/v1/questions/{questionId}` |
| 서버 | 공용 문제 등록 | 구현됨 | `POST /admin/v1/questions` |
| 서버 | 공용 문제 활성/비활성 | 구현됨 | `PATCH /admin/v1/questions/{questionId}/status` |
...
| 서버 | 사용자 문제 생성 통계 조회 | 구현됨 | `GET /admin/v1/monitoring/question-generations` |
| 배치 | 만료 시험 자동 제출 | 구현됨 | 초 단위 스케줄 |
| 배치 | 30일 미로그인 계정 정리 | 미구현 | 새 요구사항 기준 재구현 필요 |
| 배치 | 일별 통계 집계 | 구현됨 | 사용자 로그인 성공, 시험 시작, 문서 등록, 개인 문제 생성 완료 이벤트를 기준으로 daily_activity_stat 적재 |
| 공통 | JWT 생성 / 검증 | 구현됨 | 사용자/관리자 분리 시크릿, 필터 검증 |
| 공통 | Spring Security 경로 정책 | 구현됨 | 사용자/관리자 경로 보호, CORS 포함 |
| 공통 | Redis 토큰 / 시험 데이터 관리 | 구현됨 | refresh token 저장, access token blacklist |
| 공통 | OpenAI 연동 | 구현됨 | 임베딩, 개인 문제 생성 |
| 공통 | pgvector 연동 | 구현됨 | `document_chunk.embedding` 저장 및 인덱스 |
| 테스트 | 서버 테스트 코드 | 구현됨 | 도메인/서비스/API 관련 테스트 추가 |
