...
| [배포 가이드.md](./배포 가이드.md) | GitHub Actions, Docker Hub, Vultr 기반 운영 배포 절차 |
| [장애 처리 로그.md](./장애 처리 로그.md) | 운영 및 개발 중 발생한 장애 원인, 처리 내용, 재발 방지 기록 |
...
현재 서버 구조는 `tmk-core`(도메인), `tmk-infra`(영속화), `tmk-api`(사용자 API + 관리자 API 단일 Boot 서버), `tmk-batch`(배치), `tmk-user-web`(사용자 웹), `tmk-admin-web`(관리자 웹) 기준으로 구현되어 있습니다.
...
민감 정보는 코드에 두지 않고 환경 변수로 주입합니다. 로컬 개발은 직접 환경 변수를 지정하고, 운영 배포는 GitHub Actions가 개별 GitHub Secrets를 조합해 배포 시점에 `.env` 파일을 생성합니다.
