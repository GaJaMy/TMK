...
[HARD] When any implementation task is requested via `/moai` or natural language:
1. ALWAYS run the plan phase first (`/moai plan`) to create a SPEC document
2. NEVER skip to implementation (run phase) without a SPEC document existing
3. After implementation completes, ALWAYS run sync phase which auto-commits (auto_commit: true in git-strategy.yaml)

This applies to: new features, refactoring, adding endpoints, configuration changes, any code modification request.
Skip ONLY for: `/moai fix`, `/moai loop` (bug fixes that don't need a spec), `/moai sync`, `/moai review`.
