...
public class AdminAuthUseCase {
    ...
    @Transactional
    public AdminLoginResponse reissue(AdminReissueRequest request) {
        AdminLoginResult result = adminAuthService.reissue(request.refreshToken());
        return AdminLoginResponse.from(result);
    }

    @Transactional
    public void logout(Long principalId, String accessToken) {
        adminAuthService.logout(principalId, accessToken);
    }
}
