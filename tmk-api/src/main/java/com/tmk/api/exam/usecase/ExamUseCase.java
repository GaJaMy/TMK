...
public class ExamUseCase {
    ...
    public ExamDetailResult getExam(Long examId, Long userId) {
        ...
        Map<Long, Question> questionMap = buildQuestionMap(exam);

        List<ExamQuestionResult> questionResults = exam.getExamQuestions().stream()
                .map(eq -> {
    ...
    public void saveAnswers(Long examId, Long userId, List<AnswerCommand> answers) {
        Map<Long, String> answerMap = answers.stream()
                .collect(Collectors.toMap(AnswerCommand::getQuestionId, AnswerCommand::getAnswer));
    ...
    public SubmitResult submit(Long examId, Long userId) {
        ...
        int correctCount = countCorrect(exam);
    ...
    public HistoryListResult getHistory(Long userId, int page, int size) {
        ...
        long totalElements = getExamHistoryService.count(userId);
        ...
        return new HistoryListResult(summaries, page, size, totalElements, totalPages);
    }
    ...
    public HistoryDetailResult getHistoryDetail(Long examId, Long userId) {
        ...
        List<HistoryQuestionResult> questionResults = exam.getExamQuestions().stream()
                .map(eq -> {
                    Question q = questionMap.get(eq.getQuestionId());
                    List<OptionResult> options = q.getOptions().stream()
                            .map(o -> new OptionResult(o.getOptionNumber().intValue(), o.getContent()))
    ...
    private Map<Long, Question> buildQuestionMap(Exam exam) {
        return exam.getExamQuestions().stream()
                .collect(Collectors.toMap(
                        ExamQuestion::getQuestionId,
                        eq -> questionPort.findById(eq.getQuestionId())
                                .orElseThrow(() -> new BusinessException(ErrorCode.QUESTION_NOT_FOUND))
                ));
    }

    private int countCorrect(Exam exam) {
        return (int) exam.getExamQuestions().stream()
